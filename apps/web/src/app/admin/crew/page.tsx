  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-[22px] font-bold text-admin-text">Crew</h1>
          <p className="text-admin-text-secondary text-sm mt-0.5">Personas que aparecen en los vídeos</p>
        </div>
      ...
      {/* Crew */}
      <section className="space-y-2">
        <h2 className="text-xs font-semibold text-admin-text-muted uppercase tracking-widest px-1">Crew</h2>
        ...
        footer={
          <>
            ...
            <div className="flex gap-3">
              <Button theme="light" type="submit" form="crew-form" variant="primary" size="md" loading={saving} className="flex-1 justify-center">
      ...
      >
        <form id="crew-form" onSubmit={handleSave} className="space-y-4">
          <Field label="Nombre *">
            ...
            <tr key={m.id} className="hover:bg-admin-hover transition-colors">
              <td className="px-4 py-3">
                <div className="flex items-center gap-3">
                  {m.avatar_url ? (
                    <img src={m.avatar_url} alt="" className="w-8 h-8 rounded-full object-cover bg-admin-border-soft shrink-0" />
                  ) : (
                    <div className="w-8 h-8 rounded-full bg-admin-border-soft shrink-0 flex items-center justify-center text-admin-text-tertiary text-xs font-bold">
                      {m.name.charAt(0).toUpperCase()}
                    </div>
                  )}
                  <span className="text-admin-text font-medium">{m.name}</span>
                </div>
              </td>
              <td className="px-4 py-3 hidden md:table-cell">
                <span className="text-admin-text-tertiary text-xs font-mono">{m.slug}</span>
              </td>
              <td className="px-4 py-3">
...
function RoleBadge({ role }: { role: 'socio' | 'crew' }) {
  return role === 'socio' ? (
    ...
    <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-admin-border-soft text-admin-text-muted">Crew</span>
  );
}
