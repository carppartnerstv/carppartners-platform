...
| GET | `/videos?limit&offset&category&series&q&sort` | JWT + suscripción | `{videos, limit, offset}` — `sort=recent` ordena por `created_at DESC` de todo el catálogo (sin agrupar por serie); alimenta la fila de Home "Añadido recientemente", con `limit=20` |
...
| GET | `/series?category&curated` | JWT | `{series}` — solo series de primer nivel (`parent_series_id IS NULL`); `episode_count` suma los episodios propios + los de todas sus temporadas. `curated=true` filtra por `is_curated = true` sin límite (fila de Home "Mejores seleccionados para ti", marcado a mano desde `/admin/series`). Sin ese parámetro, orden y comportamiento de siempre (`order_index, title`, sin límite) — no afecta a Explorar ni a las filas por categoría |
...
| GET | `/admin/series?category` | JWT + admin | `{series}` — TODAS las series (incluidas las temporadas), con `parent_series_id`, `parent_title`, `season_count`, `video_count`, `is_curated` |
| POST | `/admin/series` `{title,slug,description?,categoryId?,seasonNum?,coverUrl?,orderIndex?,parentSeriesId?,isCurated?}` | JWT + admin | `{series}` 201 — `parentSeriesId` convierte la serie en una temporada de otra (solo 1 nivel de profundidad: el padre no puede tener a su vez padre, y una serie con temporadas propias no puede recibir padre). `isCurated` marca la serie para la fila de Home "Mejores seleccionados para ti" — a diferencia de `videos.is_featured` (un único destacado, con índice único parcial en BD), aquí pueden marcarse varias a la vez, sin límite ni lógica de "desmarcar las demás" |
| PUT | `/admin/series/:id` | JWT + admin | `{series}` — mismas reglas de `parentSeriesId`; enviar `null` explícito quita la serie madre. `isCurated` como en el POST — el toggle vive como botón directo en la fila de la tabla de `/admin/series` (icono de estrella), sin pasar por el modal de edición, igual que el destacado de vídeos |
...
| `/home` | Home tipo Netflix: hero + "Continuar viendo" + "Añadido recientemente" (`GET /videos?sort=recent&limit=20` — a nivel de vídeo/episodio, no de serie como el resto de filas; badge "Nuevo" en lo publicado en los últimos 45 días) + "Mejores seleccionados para ti" (`GET /series?curated=true`, marcado a mano desde `/admin/series`) + filas por categoría — una tarjeta por serie/película, no vídeos sueltos. Las filas nuevas se ocultan solas si no tienen elementos (mismo `VideoRow`/`SeriesRow` de siempre) | suscriptores |
